# IIS10 best practices script
# Copyright Far Edge Technology, 2015
#
# Based on work by Alexander Hass, Copyright 2014
...
$cipherSuitesOrder = @(
   # GCM (AEAD) based ciphers appear first, as they are required for HTTP2 support (it blacklists anything using CBC)
  'TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384',
  'TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256',
  'TLS_DHE_RSA_WITH_AES_256_GCM_SHA384',
  'TLS_DHE_RSA_WITH_AES_128_GCM_SHA256',
  
  # ECDHE_ECDSA require DSA server certificates and are not supported very well, but provide excellent encryption
  # Longer curve lengths are commented out (P521) to keep the size of the registry key managable (supposed to be less than 1024 characters)
  ...
  # Below are not supported in HTTP2, and lose AEAD support
  ...
  # These have no perfect forward secrecy.
  # But are required for older clients (eg: Win XP, Java 6)
...
Write-Host ''
Write-Host 'Please also confirm connections to your server in all major browsers'
Write-Host "--------------------------------------------------------------------------------`n"
